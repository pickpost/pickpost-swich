...
import { EXPOSURE_KEY } from '../lib/const.js';
...
// built-in plugins
import NetworkPlugin from '../network/network.js';

const FIREWORM_ID = '#__fireworm';
...
class Fireworm {
	...
	constructor(opt) {
		if ($.one(FIREWORM_ID)) {
			console.debug('Fireworm is already exists.'); // eslint-disable-line
			return;
		}
		// fixed the bizlog bugs bizlog会监听click事件，一直往上遍历，到body停止, fireworm是插在body外面的，所以遍历到document上的时候会报错
   ...
   */
	_addBuiltInPlugins() {
		...
		const plugins = {
			'network': {proto: NetworkPlugin, name: 'Network'},
		};
		if (!!list && tool.isArray(list)) {
			for (let i=0; i<list.length; i++) {
				...
				if (tab) {
					...
					console.debug('Unrecognized default plugin ID:', list[i]); // eslint-disable-line
				}
			}
		}
	}
   ...
   */
	_render() {
		...
		if (switchX || switchY) {
			// check edge
			if (switchX + $switch.offsetWidth > document.documentElement.clientWidth) {
				switchX = document.documentElement.clientWidth - $switch.offsetWidth;
			}
			if (switchY + $switch.offsetHeight > document.documentElement.clientHeight) {
				switchY = document.documentElement.clientHeight - $switch.offsetHeight;
			}
	...
	_bindEvent() {
		...
		$.bind($switch, 'touchmove', function(e) {
			if (e.touches.length > 0) {
				...
				// check edge
				if (x + $switch.offsetWidth > document.documentElement.clientWidth) {
					x = document.documentElement.clientWidth - $switch.offsetWidth;
				}
		...
		// show tab box
		$.delegate($.one('.vc-tabbar', that.$dom), 'click', '.vc-tab', function() {
		...
		$.bind($.one('.vc-panel', that.$dom), transitionEvent, function(e) {
		...
		$.bind($content, 'touchend', function () {
			preventMove = false;
		});

		// prevent topbar scrolling event to background
		const $topbar = $.one('div.vc-topbar', that.$dom);
		$.bind($topbar, 'touchstart' , (e) => {
			e.preventDefault();
		});
	}
   ...
   */
	...
	/**
   * trigger a Fireworm.option event
   * @protect
   */
   ...
   * add a new plugin
   * @public
   * @param object FirewormPlugin object
   * @return boolean
   */
	addPlugin(plugin) {
		// ignore this plugin if it has already been installed
		if (this.pluginList[plugin.id] !== undefined) {
			console.debug('Plugin ' + plugin.id + ' has already been added.'); // eslint-disable-line
			return false;
		}
		this.pluginList[plugin.id] = plugin;
		// init plugin only if fireworm is ready
		if (this.isInited) {
   ...
   */
	removePlugin(pluginID) {
		...
		if (plugin === undefined) {
			console.debug('Plugin ' + pluginID + ' does not exist.'); // eslint-disable-line
			return false;
		}
		// trigger `remove` event before uninstall
		plugin.trigger('remove');
		// the plugin will not be initialized before Fireworm is ready,
   ...
   */
	show() {
		...
		// set 10ms delay to fix conflict between display and transition
		setTimeout(function() {
   ...
   */
	hide() {
		...
		$.bind($mask,transitionEvent, function() {
   ...
   */
	setOption(keyOrObj, value) {
		if (tool.isString(keyOrObj)) {
			...
			console.debug('The first parameter of fireworm.setOption() must be a string or an object.'); // eslint-disable-line
		}
	}
